...
function getInstallPath(): string {
  if (process.platform === "win32") {
    ...
    return join(dir, "m365-teams-mcp.exe");
  }
  return join(homedir(), ".local", "bin", "m365-teams-mcp");
}
...
function mergeConfig(configPath: string, installPath: string): void {
  ...
  (config.mcpServers as Record<string, unknown>)["m365-teams"] = {
    command: installPath,
  };
...
export async function runSetup(): Promise<void> {
  ...
  console.log("M365 Teams MCP Setup");
  console.log("====================\n");
  ...
  console.log("  2. All 19 Teams tools will be available");
...
export async function runUninstall(): Promise<void> {
  const installPath = getInstallPath();
  const MCP_KEY = "m365-teams";
  const cacheDir = join(homedir(), ".m365-teams-mcp");

  console.log("M365 Teams Uninstall");
  console.log("====================\n");
