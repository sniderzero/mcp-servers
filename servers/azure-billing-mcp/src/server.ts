...
import { AzureApiError } from "./api/client.js";

export function createServer(client: AzureClient): Server {
  const server = new Server(
    { name: "azure-billing-mcp", version: "1.0.0" },
    { capabilities: { tools: {} } }
  );
    ...
    try {
      ...
      if (err instanceof AzureApiError) {
        throw new McpError(
          ...
          `Azure API error ${err.status}: ${err.body}`,
        );
      }
      throw err;
    }
