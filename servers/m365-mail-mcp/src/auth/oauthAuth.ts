...
function waitForCode(port: number): Promise<string> {
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (code) {
        res.end(
          "<html><body><h2>M365 Mail: Authorization successful!</h2><p>You may close this tab.</p></body></html>"
        );
        server.close();
        resolve(code);
      } else {
...
export async function doBrowserAuth(scopes: string[]): Promise<string> {
  ...
  const port = parseInt(process.env.MAIL_MCP_REDIRECT_PORT ?? String(DEFAULT_PORT), 10);
  ...
  process.stderr.write(`\n[M365 Mail] Open this URL to sign in:\n${authUrl}\n\n`);
  openBrowser(authUrl);
...
export async function getAccessToken(
  ...
  if (!options?.interactive) {
    throw new Error(
      "[M365 Mail] No cached tokens or refresh failed. Run with --auth to authenticate:\n" +
        "  m365-mail-mcp --auth"
    );
  }
