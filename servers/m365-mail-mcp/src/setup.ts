...
function getInstallPath(): string {
  if (process.platform === "win32") {
    const dir = join(process.env.LOCALAPPDATA ?? join(homedir(), "AppData", "Local"), "Programs", "m365-mail");
    return join(dir, "m365-mail.exe");
  }
  return join(homedir(), ".local", "bin", "m365-mail");
}
...
function mergeConfig(configPath: string, installPath: string): void {
  ...
  (config.mcpServers as Record<string, unknown>)["m365-mail"] = {
    command: installPath,
  };
...
export async function runSetup(): Promise<void> {
  ...
  console.log("  2. All 34 Mail tools will be available");
...
export async function runUninstall(): Promise<void> {
  const installPath = getInstallPath();
  const MCP_KEY = "m365-mail";
  const cacheDir = join(homedir(), ".m365-mail-mcp");

  console.log("M365 Mail Uninstall");
  console.log("===================\n");
