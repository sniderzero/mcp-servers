...
export const AUTH_TOOL_DEFINITIONS = [
  {
    name: "m365_mail_auth_status",
    description:
      "Check whether the M365 Mail MCP server is currently authenticated with Microsoft 365",
    inputSchema: { type: "object" as const, properties: {} },
    annotations: { readOnlyHint: true, destructiveHint: false },
  },
  {
    name: "m365_mail_auth_login",
    description:
      "Authenticate or re-authenticate the M365 Mail MCP server with Microsoft 365. Opens a browser window for sign-in — no terminal required.",
    inputSchema: { type: "object" as const, properties: {} },
...
export async function handleAuthStatus(_args: unknown, _provider: TokenProvider): Promise<unknown> {
  try {
    ...
    return { authenticated: false, message: "Not authenticated or session expired. Call m365_mail_auth_login to sign in." };
  }
}
...
export async function handleAuthLogin(_args: unknown, _provider: TokenProvider): Promise<unknown> {
  try {
    ...
    return { authenticated: true, message: "Authentication successful! Tokens have been cached. All M365 Mail tools are now available." };
  } catch (e) {
