...
function getInstallPath(): string {
  if (process.platform === "win32") {
    const dir = join(process.env.LOCALAPPDATA ?? join(homedir(), "AppData", "Local"), "Programs", "m365-sharepoint");
    return join(dir, "m365-sharepoint.exe");
  }
  return join(homedir(), ".local", "bin", "m365-sharepoint");
}
...
function mergeConfig(configPath: string, installPath: string): void {
  ...
  (config.mcpServers as Record<string, unknown>)["m365-sharepoint"] = {
    command: installPath,
  };
...
export async function runSetup(): Promise<void> {
  ...
  console.log("M365 SharePoint Setup");
  console.log("=====================\n");
  ...
  console.log("  2. All 32 SharePoint tools will be available");
...
export async function runUninstall(): Promise<void> {
  const installPath = getInstallPath();
  const MCP_KEY = "m365-sharepoint";
  const cacheDir = join(homedir(), ".m365-sharepoint-mcp");

  console.log("M365 SharePoint Uninstall");
  console.log("=========================\n");
