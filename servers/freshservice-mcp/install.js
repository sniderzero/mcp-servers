...
/**
 * Cross-platform installer for FreshService MCP Server
...
async function main() {
  console.log("\n======================================");
  console.log("  FreshService MCP Server Installer");
  console.log("======================================\n");
  ...
  console.log("(Find your API key in FreshService → avatar → Profile Settings → API Settings)\n");

  const apiKey = (await ask("FreshService API Key: ")).trim();
  if (!apiKey) { console.error("❌  API key cannot be empty."); process.exit(1); }

  const domain = (await ask("FreshService Domain (e.g. 'mycompany' from mycompany.freshservice.com): ")).trim();
  if (!domain) { console.error("❌  Domain cannot be empty."); process.exit(1); }
    ...
    console.log(JSON.stringify({
      mcpServers: {
        freshservice: {
          command: nodePath,
          args: [serverPath],
          env: { MCP_TRANSPORT: "stdio", FRESHSERVICE_API_KEY: apiKey, FRESHSERVICE_DOMAIN: domain }
        }
      }
    }, null, 2));
    process.exit(0);
  ...
  if (claudeInstalled) {
    try {
      ...
      execSync(`${claudeCmd} mcp add-json --scope user freshservice '${mcpJson}'`, { stdio: "inherit" });
      console.log("✅  Claude Code registered (available in all projects)");
    } catch {
      console.log("⚠️   Could not auto-register with Claude Code — see INSTALL.md for manual steps.");
    }
  } else {
  ...
  console.log("======================================\n");
  console.log("👉  Restart Claude Desktop to activate FreshService.");
  console.log("    The connector will appear in the connector menu.");
  if (claudeInstalled) {
    console.log("👉  FreshService tools are now available in all Claude Code sessions.\n");
  } else {
    console.log("");
  }
}
