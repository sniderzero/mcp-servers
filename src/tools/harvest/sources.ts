...
export function registerSourcesTools(server: McpServer, client: HarvestClient): void {
  server.tool(
    "greenhouse_harvest_sources_list",
    ...
    async ({ on_behalf_of_user_id, ...params }) => {
      const result = await client.get("/sources", params as Record<string, unknown>, on_behalf_of_user_id);
